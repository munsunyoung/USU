	...
	$.ajax({
		...
		success:(rslt)=>{
            ...
            for(let i=0; i<rslt.length; i++){
                let oneData = rslt[i];
                let ckData = "20"+oneData.boDate; 
                ckData = ckData.replaceAll("/","-");
                let whEvent = {
                    id: oneData.boNo,
                    title: oneData.boTitle,
                    start:ckData ,
                    end:ckData,
                    backgroundColor:"black",
                    textColor:"yellow",                
                }
                console.log("체킁:",whEvent);
                schData.push(whEvent);
            }
*/
            calDraw(rslt);
		}
...
function boardData(){
	$.getJSON(`/staff/mainPage/commonBoardList`, function (resp) {
    ...
    $.getJSON(`/staff/mainPage/commonNoticeList`, function (resp) {
...
function moveToCommonBoard(boNo){
	location.href=`/staff/board/commonNoticeView/${boNo}`;
}
...
function moveToNoticeBoard(boNo){
	location.href=`/staff/board/noticeView/${boNo}`;
}
